...
const SubCarousel = (props) =>{
    ...
    const goLeft =()=>{
        (x === 0 ? setX(-26.5 * (cards.length - 2)) : setX(x + 26.5));
        (curr === 0 ? setCurr(cards.length - 2) : setCurr(curr - 1));
        
    };
    const goRight = () =>{
        (x === -26.5 * (cards.length - 2)  ? setX(0) : setX(x - 26.5));
        (curr === cards.length - 2 ? setCurr(0) : setCurr(curr + 1));
    };
