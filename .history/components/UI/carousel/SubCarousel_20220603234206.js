...
const SubCarousel = (props) =>{
    ...
    const goLeft =()=>{
        // (x === 0 ? setX(-27.5 * (cards.length )) : setX(x + 27.5));
        (x === 0 ? setX(cards.length) : setX(x + 27.5));
        (curr === 0 ? setCurr(cards.length) : setCurr(curr - 1));
        
    };
    const goRight = () =>{
        // (x === -27.5 * (cards.length)  ? setX(0) : setX(x - 27.5));
        console.log(x);
        (curr === cards.length  ? setCurr(0) : setCurr(curr + 1));
    };
    ...
    <div className={styles.slider}>
        <div className={styles.slide}>
            ...
            {cards.map((card,index)=>{
                return(
                    <div key={index} className={styles.card} style={{transform:`translateX(${x * 10}%)`}}>
                        <SmallCard />
                    </div>
