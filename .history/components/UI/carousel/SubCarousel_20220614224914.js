...
const SubCarousel = (props) =>{
    ...
    const [x, setX]= useState(10);               //  Amount of transforming the element
    ...
    const subGoLeft = () =>{
        ...
        // (x === 0 ? setX(-(cards.length - 2)) : setX(x + 1));
        ( x === 0 ? setX( -100 * (props.cards.length-4)) : setX(x + 100) );
        // ( x === -100 * (props.cards.length -4) ? setX( -100 * (props.cards.length-4)) : setX(x + 100) );
    };
    const subGoRight = () =>{
        // (x === -100 * (cards.length - 4)  ? setX(0) : setX(x - 100));
        // (x === -(cards.length - 2) ? setX(0) : setX(x - 1));
        ( x === -100 * (props.cards.length -4) ? setX(0) : setX(x - 100) );
        // ( x === 0 && setX(0) );
        console.log(x);
    };
